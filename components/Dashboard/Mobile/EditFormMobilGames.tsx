...
import AlertsUpdateMobile from "@/components/Notif/AlertEditMobile";

interface EditFormMobileProps {
  id: string;
  slug: string;
  title: string;
  image: string;
...
export default function EditFormMobileGames({
  id,
  slug: initialSlug,
  title: initialTitle,
  image: initialImage,
    ...
    <div className="flex justify-center items-center max-sm:h-1/2 h-screen md:h-screen">
      ...
      {showAlert && (
          <div className="fixed top-16 right-5 max-md:h-10 max-md:right-0">
            <AlertsUpdateMobile title={title} />
          </div>
        )}
