...
export const metadata: Metadata = {
  ...
  const getMobileById = async (id: any) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/mobilegames/${id}`, {
        cache: "no-store",
      });
...
async function page({ params }: any) {
  ...
  return (
    <>
        <EditFormMobileGames  id={id} slug={slug} title={title} image={image} price={price} />
    </>
  )
}
