...
export const metadata: Metadata = {
  ...
  const getPcGamesById = async (id: any) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/pcgames/${id}`, {
        cache: "no-store",
      });
...
async function page({params}: any) {
  ...
  return (
    <>
        <EditFormPcGames  id={id} slug={slug} title={title} image={image} price={price} />
    </>
  )
}
