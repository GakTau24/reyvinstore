...
async function getDetailMobile(slug: string) {
  const data = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/mobilegames/${slug}`, {cache: "no-store"});
  return data.json();
}
      ...
      <div className="max-w-sm rounded-lg shadow">
        ...
        <div className="p-3 text-center">
          <Link href={`https://wa.me/6285173125847`}>
            <h5 className="mb-2 text-xl font-bold tracking-tight dark:text-slate-300">{res.mobileGame.title}</h5>
          </Link>
          ...
          <pre className="mb-3 font-normal text-left dark:text-slate-300">{res.mobileGame.price}</pre>
          <hr className="my-3 border-gray-700 sm:mx-auto dark:border-gray-300 lg:my-4 opacity-20" />
          <motion.div
